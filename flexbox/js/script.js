...
function getElement(selector) {
    const element = document.querySelector(selector);
        ...
        if(!element) {
            console.warn(`Elmento não encontrado: ${selector}`);
        }
...
function setActiveButton(button, selector) {
        document.querySelectorAll(selector).forEach(currentButton => {
            ...
            currentButton.setAtribute("aria-pressed", "false");
        });

        button.classList.add("active");
        button.setAtribute("aria-pressed", "true");
    }
...
*/
 function createController({
    ...
    const target = getElment(targetSelector);
    const output = getElment(outputSelector);
    const description = getElment(descriptionSelector);
    const buttons = getElment(buttonSelector);
    ...
    buttons.forEach(button => {
        button.addEventListener("click", () => {
            ...
            setActiveButton(button, butttonSelector);
